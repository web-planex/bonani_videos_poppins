...
if (!empty($_POST['update_langs'])) {
    ...
    $query = mysqli_query($mysqli, "UPDATE `config` SET `value` = '" . time() . "' WHERE `name` = 'last_update'");
	$name  = md5(microtime()) . '_updated.php';
    rename('run_update.php', $name);
	/*
    $data  = array();
            ...
            <div class="col-md-10">
               <div class="wo_install_wiz">
                 <?php if ($updated == false) { ?>
                  <div>
                     <h2 class="light">Update to v<?php echo $new_version;?> </h2>
                     ...
                     <?php if ($updated == true) { ?>
                      <div>
                        <div class="empty_state">
                           ...
                           <br>
                           <a href="<?php echo $pt->config->site_url; ?>" class="btn btn-main" style="line-height:50px;">Home</a>
                        </div>
                     </div>
                     <?php }?>
                  </div>
...
<script>  
var queries = [
    'UPDATE `config` SET `value` = "<?php echo $new_version; ?>" WHERE `name` = "version";',    
];
    ...
    $.post('?update', {
        ...
        if (queriesLength > count) {
            ...
            $.post('?run_lang', {
              ...
              $('#button-update').attr('onclick','window.location="<?php echo $pt->config->site_url; ?>"').html('Home <svg viewBox="0 0 19 14" xmlns="http://www.w3.org/2000/svg" width="18" height="18"> <path fill="currentColor" d="M18.6 6.9v-.5l-6-6c-.3-.3-.9-.3-1.2 0-.3.3-.3.9 0 1.2l5 5H1c-.5 0-.9.4-.9.9s.4.8.9.8h14.4l-4 4.1c-.3.3-.3.9 0 1.2.2.2.4.2.6.2.2 0 .4-.1.6-.2l5.2-5.2h.2c.5 0 .8-.4.8-.8 0-.3 0-.5-.2-.7z"></path> </svg>');
              $('#button-update').attr('disabled', false);
              $(".wo_update_changelog").scrollTop($(".wo_update_changelog")[0].scrollHeight);
              $('body').attr('data-update', 'true');

              setTimeout(function(){
                window.location.href="<?php echo $pt->config->site_url; ?>";                  
              },1500);
